#!/usr/bin/env python3
"""Provider-neutral bridge for the Technology Library spreadsheet queue.

Consumes supported files from TO_REVIEW/SPREADSHEETS, runs local selective
spreadsheet extraction, writes packages to READY_FOR_ANALYSIS, persists the
provider-neutral PROCESS_LOG record, and moves originals to the spreadsheet
PROCESSED queue. Unsupported legacy spreadsheet formats remain untouched.

Normal stdout contains only counts and package IDs, never private filenames,
...
from spreadsheet_ingest import (
    MAX_SUMMARY_BYTES,
    SPREADSHEET_PIPELINE_VERSION,
    SPREADSHEET_SUMMARY_VERSION,
    SUPPORTED_SUFFIXES,
)

DEFAULT_INBOX = "99_INBOX/TO_REVIEW/SPREADSHEETS"
DEFAULT_READY = "99_INBOX/READY_FOR_ANALYSIS"
DEFAULT_PROCESSED = "99_INBOX/PROCESSED/SPREADSHEETS"
DEFAULT_ERROR = "99_INBOX/ERROR/SPREADSHEETS"
...
def build_record(
    *,
    ...
    return build_process_record(
        ...
        compactor_version=SPREADSHEET_SUMMARY_VERSION,
        status=status,
...
def process_one(item: dict[str, Any], args: argparse.Namespace, workspace: Path) -> dict[str, Any]:
    ...
    command = [
        sys.executable,
        str(Path(__file__).with_name("spreadsheet_ingest.py")),
        str(local_source),
        "--out", str(package_dir),
        "--source-id", source_id,
    ...
    if result.returncode != 0:
        ...
        persist_error_record(
            ...
            pid=pid,
            destination=destination,
            error_code="spreadsheet_processing_failed",
        )
        return {"package_id": pid, "status": "error", "error_code": "spreadsheet_processing_failed"}
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Ingest routed spreadsheets from private rclone storage.")
    ...
    if not shutil.which("rclone"):
        print("spreadsheet_bridge_error code=rclone_missing")
        return 2
    ...
    for folder in (args.inbox, args.ready, args.processed, args.error, args.process_log):
        if not ensure_remote_dir(args.remote, folder):
            print("spreadsheet_bridge_error code=remote_directory_unavailable")
            return 2

    pending = list_supported(args.remote, args.inbox)
    if pending is None:
        print("spreadsheet_bridge_error code=list_failed")
        return 2
    selected = pending[:args.batch_size]
    if not selected:
        print("spreadsheet_bridge_ok pending=0 processed=0 finalized=0 errors=0")
        return 0
    ...
    (workspace / "spreadsheet-bridge-summary.json").write_text(
        json.dumps(summary, ensure_ascii=False, indent=2) + "\n",
        encoding="utf-8",
    )
    print(
        "spreadsheet_bridge_ok "
        f"pending={len(selected)} processed={processed} finalized={finalized} errors={errors}"
    )
    return 0
