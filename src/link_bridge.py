#!/usr/bin/env python3
"""Provider-neutral bridge for the Technology Library link queue.

Consumes .url/.webloc shortcuts, runs Link V1 entirely offline, uploads compact
packages, persists PROCESS_LOG records and moves originals to PROCESSED/LINKS.
Deterministically unsafe/invalid links are quarantined in ERROR/LINKS as
non-retryable rejections without failing the whole batch.

Normal stdout contains counts and package IDs only, never private filenames or
full URLs.
"""
...
from link_ingest import (
    LINK_PIPELINE_VERSION,
    LINK_SUMMARY_VERSION,
    MAX_SUMMARY_BYTES,
    SUPPORTED_SUFFIXES,
    ingest_link,
)
...
DEFAULT_INBOX = "99_INBOX/TO_REVIEW/LINKS"
DEFAULT_READY = "99_INBOX/READY_FOR_ANALYSIS"
DEFAULT_PROCESSED = "99_INBOX/PROCESSED/LINKS"
DEFAULT_ERROR = "99_INBOX/ERROR/LINKS"
...
MIME_SUFFIXES = {
    ...
    "application/x-webloc": ".webloc",
}

NONRETRYABLE_LINK_ERRORS = {
    "source_empty",
    "source_too_large",
    "unsupported_link_format",
    "link_decode_failed",
    "link_parse_failed",
    "link_url_missing",
    "link_url_invalid",
    "link_scheme_not_allowed",
    "link_userinfo_not_allowed",
    "link_host_missing",
    "link_host_invalid",
    "link_port_invalid",
}
...
def build_record(
    *,
    ...
    return build_process_record(
        ...
        compactor_version=LINK_SUMMARY_VERSION,
        status=status,
...
def reject_source(
    *,
    ...
    if moved and persisted:
        return {"package_id": pid, "status": "rejected", "reason": error_code}
    return {"package_id": pid, "status": "error", "error_code": "rejection_persist_failed"}
...
def process_one(item: dict[str, Any], args: argparse.Namespace, workspace: Path) -> dict[str, Any]:
    ...
    try:
        ingest_link(local_source, package_dir, source_id=source_id)
    except RuntimeError as exc:
        code = str(exc)
        if code in NONRETRYABLE_LINK_ERRORS:
        ...
        return {"package_id": pid, "status": "error", "error_code": "link_processing_failed"}
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Process .url/.webloc shortcuts from a private rclone queue without network fetches.")
    ...
    if not shutil.which("rclone"):
        print("link_bridge_error code=rclone_missing")
        return 2
    ...
    for folder in (args.inbox, args.ready, args.processed, args.error, args.process_log):
        if not ensure_remote_dir(args.remote, folder):
            print("link_bridge_error code=remote_directory_unavailable")
            return 2

    items = list_supported(args.remote, args.inbox)
    if items is None:
        print("link_bridge_error code=list_failed")
        return 2

    if args.only_name is not None:
        items = [item for item in items if item_name(item) == args.only_name]
        if len(items) != 1:
            print("link_bridge_error code=selected_item_missing")
            return 2
    ...
    if errors:
        print(
            "link_bridge_error code=batch_failed "
            f"pending={len(items)} selected={len(selected)} processed={processed} "
            f"finalized={finalized} rejected={rejected} errors={errors} warnings={warnings}"
        )
        return 2
    ...
    print(
        "link_bridge_ok "
        f"pending={len(items)} selected={len(selected)} processed={processed} "
        f"finalized={finalized} rejected={rejected} errors=0 warnings={warnings}"
    )
    return 0
